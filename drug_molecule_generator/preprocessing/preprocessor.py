...
class preprocess_data:
    ...
    def fit(self,X=None,Y=None):
        self.input_text_vectorization_layer = TextVectorization(max_tokens = len(config.BASE_VOCABULARY)+2,
                                                           ...
                                                           output_sequence_length = config.MAX_INPUT_SEQUENCE_LEN,vocabulary = config.BASE_VOCABULARY)

        self.output_text_vectorization_layer = TextVectorization(max_tokens = len(config.BASE_VOCABULARY)+2,
                                                           standardize=None,split = "character",
                                                           output_sequence_length = config.MAX_OUTPUT_SEQUENCE_LEN,vocabulary = config.BASE_VOCABULARY)
